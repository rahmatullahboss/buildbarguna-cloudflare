    ...
    <div className="space-y-6">
      ...
      {msg && (
        <div className="flex items-start gap-2 bg-green-50 border border-green-200 text-green-700 rounded-lg p-3 text-sm">
          ...
          <button onClick={() => setMsg('')} aria-label="বার্তা বন্ধ করুন" className="ml-auto">✕</button>
        </div>
      )}
      {errMsg && (
        <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 rounded-lg p-3 text-sm">
          <AlertTriangle size={16} className="mt-0.5 shrink-0" /> {errMsg}
          <button onClick={() => setErrMsg('')} aria-label="ত্রুটি বার্তা বন্ধ করুন" className="ml-auto">✕</button>
        </div>
      )}
