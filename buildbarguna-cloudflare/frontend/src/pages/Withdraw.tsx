...
function WithdrawalSteps({
  ...
  const methodLabel =
    method === "nagad" ? "Nagad" : method === "cash" ? "অফিস" : "bKash";
  const steps = [
      ...
      {steps.map((step, i) => {
        ...
        return (
          ...
          >
            ...
            {i < steps.length - 1 && (
              <div
                className={`flex-1 h-0.5 mx-1 mb-3 rounded-full ${
          ...
          {w.withdrawal_method !== "cash" && (
            <div className="flex justify-between">
              <span className="text-gray-500">
                {w.withdrawal_method === "nagad" ? "Nagad" : "bKash"} নম্বর
              </span>
...
function BalancePipelineCard({ balance }: { balance: BalanceData }) {
  ...
  const pendingPct = Math.min(
    100 - withdrawnPct - approvedPct,
    (pending / base) * 100,
  );
  const availablePct = Math.max(
    0,
    100 - withdrawnPct - approvedPct - pendingPct,
  );
      ...
      {approved > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-blue-700">
            <strong>{formatTaka(approved)}</strong> অনুমোদিত হয়েছে এবং শীঘ্রই
            আপনার অ্যাকাউন্টে পাঠানো হবে।
          </p>
        </div>
      )}
    </div>
...
export default function Withdraw() {
  ...
  const requestMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.success) {
        setMsg(
          `✅ উত্তোলন অনুরোধ সফলভাবে জমা হয়েছে! অ্যাডমিন ৭২ ঘণ্টার মধ্যে যাচাই করে আপডেট জানাবে।`,
        );
        setErrMsg("");
        setAmountTaka("");
        setBkashNumber("");
  ...
  const breakdownTotal = breakdownData?.success
    ? breakdownData.data.total_earned_paisa
    : 0;
  ...
  const isValidAmount = balance
    ...
    : false;
  const isValidPhone =
    withdrawalMethod === "cash" || /^01[3-9]\d{8}$/.test(bkashNumber);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-700 via-violet-600 to-purple-600 rounded-3xl p-5 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-purple-100 text-sm mt-1">
            আপনার অর্জিত মুনাফা উত্তোলন করুন। সাবমিট করার ৭২ ঘণ্টার মধ্যে
            অ্যাডমিন আপডেট করবে।
          </p>
        </div>
      </div>
      ...
      {msg && (
        <div className="flex items-start gap-2 bg-green-50 border border-green-200 text-green-700 rounded-lg p-3 text-sm">
          ...
          <button
            onClick={() => setMsg("")}
            className="ml-auto p-1 hover:bg-green-100/50 rounded-full transition-colors"
      ...
      {errMsg && (
        <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 rounded-lg p-3 text-sm">
          ...
          <button
            onClick={() => setErrMsg("")}
            className="ml-auto p-1 hover:bg-red-100/50 rounded-full transition-colors"
      ...
      {balanceLoading ? (
        ...
        balance && <BalancePipelineCard balance={balance as any} />
      )}
      ...
      {breakdown.length > 0 && (
        <div className="card">
          <h2 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
            <Layers size={18} className="text-indigo-500" /> আয়ের উৎস (খাত
            অনুযায়ী)
          </h2>
          <div className="space-y-2.5">
            {breakdown.map((item: IncomeBreakdownItem, i: number) => {
              ...
              const icon = isProject
                ...
                : isRef
                  ? "🎁"
                  : item.source === "monthly_earnings"
                    ? "📈"
                    : item.source === "capital_refund"
                      ? "🏦"
                      : "💰";
              ...
              return (
                <div
                  key={`${item.source}-${item.project_id ?? "ref"}-${i}`}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-right shrink-0 ml-3">
                      <p
                        className={`text-sm font-bold ${textColors[i % textColors.length]}`}
                      >
      ...
      {hasPending && (
        <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-3 text-sm">
          ...
          আপনার একটি উত্তোলন অনুরোধ অপেক্ষমাণ আছে। অনুমোদনের পরে নতুন অনুরোধ
          করতে পারবেন।
        </div>
      )}
      ...
      {/* Withdrawal form */}
      <div
        className={`card ${hasPending ? "opacity-60 pointer-events-none" : ""}`}
      >
        <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
          <ArrowDownCircle size={20} className="text-primary-500" /> নতুন
          উত্তোলন অনুরোধ
        </h2>

        {balance && (
          <p className="text-xs text-gray-500 mb-4 bg-gray-50 rounded-lg px-3 py-2">
            সর্বনিম্ন {formatTaka(balance.settings.min_paisa)} • প্রতি{" "}
            {balance.settings.cooldown_days} দিনে একবার
          </p>
        )}
        ...
        <div className="space-y-4">
          <div>
            ...
            {amountTaka && balance && amountPaisa > balance.available_paisa && (
              <p className="text-xs text-red-500 mt-1">
                উপলব্ধ ব্যালেন্সের বেশি হতে পারবে না
              </p>
            )}
          </div>
          ...
          {withdrawalMethod !== "cash" && (
            <div>
              <label className="label">
                {withdrawalMethod === "nagad" ? "Nagad" : "bKash"} নম্বর
              </label>
              <input
              ...
              <p className="text-xs text-gray-500 mt-2">
                ...
                রিকোয়েস্ট করার ৭২ ঘণ্টার মধ্যে অ্যাডমিন আপনার রিকোয়েস্ট যাচাই
                করে অ্যাকাউন্টে ব্যালেন্স আপডেট করে দিবে।
              </p>
            </div>
          )}
          ...
          {withdrawalMethod === "cash" && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-700 flex items-start gap-2">
              ...
              <div>
                <p>
                  ক্যাশ উত্তোলন অনুমোদনের পরে অফিস থেকে সংগ্রহ করুন। মোবাইল
                  নম্বর প্রয়োজন নেই।
                </p>
                <p className="mt-1 text-xs font-semibold">
                  রিকোয়েস্ট করার ৭২ ঘণ্টার মধ্যে অ্যাডমিন আপনার রিকোয়েস্ট
                  যাচাই করে আপডেট করে দিবে।
                </p>
              </div>
            </div>
          )}
              ...
              <div className="flex items-start gap-2 text-amber-800 text-sm">
                ...
                <div>
                  ...
                  <p className="text-xs mt-1">
                    <span className="font-bold">{formatTaka(amountPaisa)}</span>{" "}
                    টাকা
                    {withdrawalMethod !== "cash" ? (
                      <>
                        ...
                        নম্বরে{" "}
                        {withdrawalMethod === "nagad" ? "Nagad" : "bKash"} এ
                        পাঠানোর অনুরোধ করছেন?
                      </>
                    ) : (
