...
export default function AdminWithdrawals() {
  ...
  const [completingWithdrawal, setCompletingWithdrawal] =
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.success) {
        setMsg("✅ উত্তোলন অনুমোদন করা হয়েছে");
        invalidate();
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-purple-100 text-sm mt-0.5">
              সদস্যদের উত্তোলন অনুরোধ অনুমোদন ও প্রক্রিয়া করুন
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {(["pending", "approved", "completed", "rejected", "all"] as const).map(
          (s) => (
            <button
              key={s}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            <h3 className="font-bold text-lg">
              {completingWithdrawal.withdrawal_method === "nagad"
                ? "Nagad"
                : "bKash"}{" "}
              TxID লিখুন
            </h3>
            <p className="text-sm text-gray-500">
              {completingWithdrawal.withdrawal_method === "nagad"
                ? "Nagad"
                : "bKash"}{" "}
              এ টাকা পাঠানোর পরে Transaction ID লিখুন
            </p>
            <input
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500">
                    ...
                    <span>
                      পদ্ধতি:{" "}
                      <span
                        className={`font-medium ${w.withdrawal_method === "cash" ? "text-green-600" : w.withdrawal_method === "nagad" ? "text-orange-600" : "text-pink-600"}`}
                      >
                        {w.withdrawal_method === "cash"
                          ? "💵 ক্যাশ"
                          : w.withdrawal_method === "nagad"
                            ? "📱 Nagad"
                            : "📱 bKash"}
                      </span>
                    </span>
                    {w.bkash_number && (
                      <span>
                        {w.withdrawal_method === "nagad" ? "Nagad" : "bKash"}:{" "}
                      ...
                      {w.withdrawal_method === "cash" ? (
                        <button
                          onClick={() => {
                            setMsg("");
                            setErrMsg("");
                            completeMutation.mutate({ id: w.id });
                          }}
                          disabled={
                            completeMutation.isPending ||
                            rejectMutation.isPending
