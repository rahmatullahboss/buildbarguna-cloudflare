...
PERSIST_DIR="${PERSIST_DIR:-$(mktemp -d /tmp/bb-completed-refund.XXXXXX)}"
PORT="${PORT:-8794}"
...
run_d1_sql "
  ...
  INSERT INTO member_registrations (
    ...
    'FORM-COMPLETE-001', 'Member User', 'Father', 'Mother', '1990-01-01',
  ...
  VALUES (1, 'Completed Refund Trial', 1000000, 10, 100000, 'active', 0, datetime('now'));
...
ADMIN_TOKEN="$(create_token 1 01700000000 admin@example.com admin completed-admin)"
MEMBER_TOKEN="$(create_token 2 01700000001 member@example.com member completed-member)"

COMPLETE_RESPONSE="$(api PATCH /api/admin/projects/1/status "$ADMIN_TOKEN" '{"status":"completed"}')"
