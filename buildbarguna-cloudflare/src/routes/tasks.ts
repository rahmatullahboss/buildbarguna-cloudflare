...
taskRoutes.get('/history', async (c) => {
  ...
  const countResult = batchResults[0].results?.[0] as { total: number } | undefined
  const historyResults = batchResults[1].results || []

  return ok(c, paginate(historyResults, countResult?.total || 0, page, limit))
})
...
taskRoutes.get('/', async (c) => {
  ...
  // Execute all queries in a single batch to reduce network roundtrips
  const batchResults = await c.env.DB.batch([
    ...
    // 1: Get today's completions for this user
    c.env.DB.prepare(
  ...
  const tasks = batchResults[0].results as Array<{
    id: number
    title: string
  ...
  const userPointsResult = batchResults[4].results?.[0] as { available_points: number; lifetime_earned: number; monthly_earned: number } | undefined
