...
taskRoutes.get('/history', async (c) => {
  ...
  const [countResultRes, historyResultRes] = await c.env.DB.batch<any>([
    c.env.DB.prepare(
  ...
  const countResult = countResultRes.results?.[0] as { total: number } | undefined
  const historyResult = historyResultRes.results ?? []
