...
taskRoutes.get('/history', async (c) => {
  ...
  const total = (batchResults[0].results?.[0] as { total: number } | undefined)?.total || 0
  const historyResults = batchResults[1].results

  return ok(c, paginate(historyResults, total, page, limit))
})
...
taskRoutes.get('/', async (c) => {
  ...
  // ⚡ Bolt: Use db.batch() to fetch tasks, completions, and one-time tasks in a single network call
  const batchResults = await c.env.DB.batch([
