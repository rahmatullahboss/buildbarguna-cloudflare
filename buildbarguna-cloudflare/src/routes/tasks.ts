...
taskRoutes.get('/', async (c) => {
  ...
  // ⚡ Bolt: Use db.batch() instead of sequential queries to prevent multiple HTTP network roundtrips in D1
  const batchResults = await c.env.DB.batch([
    c.env.DB.prepare(
      `SELECT
        dt.id,
  ...
  // ⚡ Bolt: Use db.batch() instead of sequential queries to prevent multiple HTTP network roundtrips in D1
  // Ensure user_points row exists (C3 FIX) and get balance
  const pointsBatch = await c.env.DB.batch([
    c.env.DB.prepare(
  ...
  const userPointsResult = pointsBatch[1].results?.[0] as { available_points: number; lifetime_earned: number; monthly_earned: number } | undefined
