...
taskRoutes.get('/history', async (c) => {
  ...
  // ⚡ Bolt: Use db.batch() instead of Promise.all or sequential queries to prevent per-query HTTP network overhead in D1
  ...
  const historyResult = batchResults[1].results;
