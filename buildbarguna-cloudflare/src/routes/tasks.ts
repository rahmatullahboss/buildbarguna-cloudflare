...
taskRoutes.get('/history', async (c) => {
  ...
  const [countRes, historyRes] = await c.env.DB.batch<any>([
    c.env.DB.prepare(
  ...
  const total = countRes.results?.[0]?.total as number || 0

  return ok(c, paginate(historyRes.results, total, page, limit))
})
