...
taskRoutes.get('/history', async (c) => {
  ...
  const countRow = results[0].results?.[0] as { total: number } | undefined
  const historyResult = results[1].results
  
  return ok(c, paginate(historyResult, countRow?.total || 0, page, limit))
})
