...
taskRoutes.get('/history', async (c) => {
  ...
  const historyResults = results[1].results
  
  return ok(c, paginate(historyResults, countRow?.total ?? 0, page, limit))
})
...
taskRoutes.get('/', async (c) => {
  ...
  // ⚡ Bolt: Combine queries using db.batch() to prevent per-query HTTP network overhead in D1
  const results = await c.env.DB.batch([
  ...
  for (const row of results[1].results) {
    const r = row as unknown as { task_id: number; count: number }
    todayCompletions.set(r.task_id, r.count)
  }

  const oneTimeResult = results[2]
