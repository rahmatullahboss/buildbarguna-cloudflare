...
taskRoutes.get('/history', async (c) => {
  ...
  return ok(c, paginate(historyResult.results, totalCount, page, limit))
})
...
taskRoutes.get('/', async (c) => {
  ...
  // ⚡ Bolt: Use db.batch() to prevent per-query HTTP network overhead in D1
  const [tasksResult, completionsResult, oneTimeResult] = await c.env.DB.batch([
    c.env.DB.prepare(
      `SELECT
        dt.id,
        dt.title,
