...
taskRoutes.get('/history', async (c) => {
  ...
  const countResult = batchResults[0].results?.[0] as unknown as { total: number } | undefined
  const historyResult = batchResults[1].results as any[]

  return ok(c, paginate(historyResult, countResult?.total ?? 0, page, limit))
})
...
taskRoutes.get('/', async (c) => {
  ...
  // ⚡ Bolt: Use db.batch() to combine multiple read queries to prevent per-query HTTP network overhead in D1
  const batchResults = await c.env.DB.batch([
    c.env.DB.prepare(
      `SELECT
        dt.id,
  ...
  const tasksResult = batchResults[0]
  const completionsResult = batchResults[1]
  const oneTimeResult = batchResults[2]
