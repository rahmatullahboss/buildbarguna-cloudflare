...
taskRoutes.get('/history', async (c) => {
  ...
  const batchResults = await c.env.DB.batch(queries)
  const countResult = (batchResults[0].results?.[0] as { total: number } | undefined) || { total: 0 }
  const historyResult = { results: batchResults[1].results || [] }
