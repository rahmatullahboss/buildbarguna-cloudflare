...
companyExpenseRoutes.post('/admin/allocate', zValidator('json', allocateSchema), async (c) => {
  ...
  if (expense.allocation_method === 'by_project_value') {
    ...
    // ⚡ Bolt: Replace N+1 queries with a single GROUP BY query for performance
    const projectIds = projects.results.map(p => p.id)
    const placeholders = projectIds.map(() => '?').join(',')

    let revenueMap = new Map<number, number>()
    if (projectIds.length > 0) {
      ...
      revenueMap = new Map(revenueData.results.map(r => [r.project_id, r.total]))
    }

    const revenues = projects.results.map(p => ({
      ...p,
