...
companyExpenseRoutes.post('/admin/allocate', zValidator('json', allocateSchema), async (c) => {
  ...
  if (expense.allocation_method === 'by_project_value') {
    ...
    if (projects.results.length > 0) {
      const stmts = projects.results.map(p =>
        c.env.DB.prepare(
      ...
      const batchResults = await c.env.DB.batch<{ total: number }>(stmts)
      revenues = projects.results.map((p, i) => {
        const revTotal = batchResults[i]?.results?.[0]?.total ?? 0
        return { ...p, revenue: revTotal }
      })
    }
