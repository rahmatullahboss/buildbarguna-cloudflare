...
companyExpenseRoutes.post('/admin/allocate', zValidator('json', allocateSchema), async (c) => {
  ...
  if (expense.allocation_method === 'by_project_value') {
    ...
    if (projects.results.length > 0) {
      const revenueResults = await c.env.DB.batch<{ total: number }>(
      ...
      revenues = projects.results.map((p, i) => ({
        ...p,
        revenue: revenueResults[i]?.results?.[0]?.total ?? 0
      }))
    }
...
companyExpenseRoutes.get('/admin/list', async (c) => {
  ...
  return ok(c, paginate(rowsRes.results as (CompanyExpense & { created_by_name: string })[], countRow?.total ?? 0, page, limit))
})
