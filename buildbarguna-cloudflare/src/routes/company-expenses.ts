...
companyExpenseRoutes.post('/admin/allocate', zValidator('json', allocateSchema), async (c) => {
  ...
  if (expense.allocation_method === 'by_project_value') {
    ...
    let revenueBatchResults: any[] = []
    if (revenueStatements.length > 0) {
      revenueBatchResults = await c.env.DB.batch(revenueStatements)
    }

    const revenues = projects.results.map((p, i) => {
      const rev = revenueBatchResults[i]?.results?.[0] as { total: number } | undefined
...
companyExpenseRoutes.get('/admin/list', async (c) => {
  ...
  const rows = batchResults[0]
  const countRow = batchResults[1].results?.[0] as { total: number } | undefined

  return ok(c, paginate(rows.results as (CompanyExpense & { created_by_name: string })[], countRow?.total ?? 0, page, limit))
})
