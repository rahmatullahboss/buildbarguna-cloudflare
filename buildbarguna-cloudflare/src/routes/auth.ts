...
authRoutes.post('/reset-password', zValidator('json', resetPasswordSchema), async (c) => {
  ...
  // Security: IP-based rate limiting to prevent token brute-forcing/scanning
  const ip = c.req.header('CF-Connecting-IP') ?? c.req.header('X-Forwarded-For') ?? 'unknown'
  const rateLimitKey = `reset_password:${ip}`
  const rateLimit = await checkRateLimit(
    c.env,
    rateLimitKey,
    RATE_LIMITS.PASSWORD_RESET.MAX_ATTEMPTS,
    RATE_LIMITS.PASSWORD_RESET.WINDOW_MINUTES * 60
  )
