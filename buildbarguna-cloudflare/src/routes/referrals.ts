...
referralRoutes.get('/stats', async (c) => {
  ...
  const recentReferrals = batchResults[2]
...
adminReferralRoutes.get('/stats', async (c) => {
  ...
  const totalBonuses = adminBatchResults[0].results?.[0] as { total_bonuses: number; total_paid_paisa: number } | undefined
  const topReferrers = adminBatchResults[1]
