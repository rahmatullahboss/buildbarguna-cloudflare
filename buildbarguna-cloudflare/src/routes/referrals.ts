...
referralRoutes.get('/stats', async (c) => {
  ...
  const recentReferralsResults = batchResults[2].results || []
  ...
  const maskedReferrals = (recentReferralsResults as Array<{
    name: string
    created_at: string
    has_invested: number
...
adminReferralRoutes.get('/stats', async (c) => {
  ...
  const totalBonuses = batchResults[0].results?.[0] as { total_bonuses: number; total_paid_paisa: number } | undefined
  const topReferrersResults = batchResults[1].results || []

  return ok(c, {
    total_bonuses_issued: totalBonuses?.total_bonuses ?? 0,
    total_bonus_paid_paisa: totalBonuses?.total_paid_paisa ?? 0,
    top_referrers: topReferrersResults
  })
})
