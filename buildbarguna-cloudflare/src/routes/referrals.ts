...
referralRoutes.get('/stats', async (c) => {
  ...
  const referredUsers = results[0].results?.[0] as { total: number } | undefined
  const bonusSummary = results[1].results?.[0] as { total_bonus_paisa: number; bonuses_earned: number } | undefined
  const recentReferrals = results[2]
  const pendingReferrals = results[3].results?.[0] as { cnt: number } | undefined
  const bonusSetting = results[4].results?.[0] as { value: string } | undefined
...
adminReferralRoutes.get('/stats', async (c) => {
  // ⚡ Bolt: Use db.batch() instead of Promise.all to prevent per-query HTTP network overhead in D1
  const resultsStats = await c.env.DB.batch([
    c.env.DB.prepare(
  ...
  const totalBonuses = resultsStats[0].results?.[0] as { total_bonuses: number; total_paid_paisa: number } | undefined
  const topReferrers = resultsStats[1]
