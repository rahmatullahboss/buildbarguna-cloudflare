...
profitRoutes.post('/distribute/:projectId', zValidator('json', distributeSchema), async (c) => {
  ...
  const distResult = await c.env.DB.prepare(
    ...
    data.period_start ?? null, data.period_end ?? null, userId
  ).run()
  ...
  const investorRateBps = (100 - data.company_share_percentage) * 100
  ...
  for (const sh of shareholders.results) {
    ...
    statements.push(
      ...
      .bind(sh.user_id, projectId, currentMonth, sh.shares_held, investorRateBps, profitAmount)
    )
  ...
  for (let i = 0; i < statements.length; i += chunkSize) {
    ...
    try {
      ...
      failedCount += (chunk.length / 4) // যেহেতু প্রতি ইউজারের ৪টি স্টেটমেন্ট
    }
  }
