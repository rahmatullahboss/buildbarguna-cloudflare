...
## 2024-03-24 - Modal Dialog Accessibility
**Learning:** Custom modals (e.g. `fixed inset-0`) lacking native dialog behaviors severely impact screen reader users as they miss the context of the pop-up container. Modals must include `role="dialog"`, `aria-modal="true"`, and ideally `aria-labelledby` referencing their heading.
**Action:** When creating or fixing custom modal components, always include `role="dialog"` and `aria-modal="true"` on the modal wrapper to announce it correctly to assistive technologies.
