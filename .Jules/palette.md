...
## 2024-03-23 - Modal Close Button Accessibility
**Learning:** Ad-hoc modal close buttons (like in TermsModal) must include localized ARIA labels and explicit focus rings, especially since icon-only buttons lack inherent context for screen readers and keyboard users.
**Action:** Use standard Bengali phrasing "বন্ধ করুন" for `aria-label` and `title` attributes on modal close actions, and apply explicit focus utility classes (`focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-primary-500`).
