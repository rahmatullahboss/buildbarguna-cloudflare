...
## 2024-05-15 - Label-Input Association Pattern
**Learning:** The application consistently uses `<label className="label">` but initially missed `htmlFor`/`id` associations. This breaks screen reader functionality.
**Action:** Always ensure any new or modified forms explicitly link `<label>` and `<input>` using matching `htmlFor` and `id` attributes. This is a quick win for accessibility.
