...
## 2026-06-23 - Accessibility for Ad-hoc Dismiss and Plus Buttons
**Learning:** Icon-only dismiss buttons (like '✕') and action buttons (like 'Plus') on alert components often lack essential screen reader attributes and visible focus states, preventing keyboard-only users from successfully interacting with them.
**Action:** Always attach standard 'aria-label', 'title', and 'focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-offset-1' to icon-only buttons to guarantee keyboard and screen reader accessibility.
