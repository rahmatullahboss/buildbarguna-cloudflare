...
## 2025-03-05 - D1 sequential query batching
**Learning:** In Cloudflare D1, sequential independent read queries in a single route handler cause redundant HTTP network roundtrips, significantly increasing latency.
**Action:** Combine sequential independent read queries into a single `c.env.DB.batch()` call to eliminate multiple network roundtrips.
